		...
		if ETWCommonLogicChecks.BrawlerShouldExecute() then
			...
			if (axeKills + longBluntKills) < SBvars.BrawlerKills then
				...
				self.labelBrawlerKillsProgress:setTooltip(
					getText("Sandbox_ETW_BrawlerKills") .. " (" .. getText("Sandbox_ETW_BrawlerKills_tooltip") .. ")"
				)
...
function ISETWProgressUI:render()
	...
	updateBar(
		self.barOutdoorsman,
		percentile(SBvars.OutdoorsmanCounter * -2, SBvars.OutdoorsmanCounter * 2, modData.OutdoorsmanSystem.OutdoorsmanCounter),
	...
	updateBar(self.barHearingSystem, percentile(0, SBvars.HearingSystemSkill, levels), getText("UI_ETW_CurrentValue") .. levels)
	levels = maintenance + carpentry + farming + firstAid + electrical + metalworking + mechanics + tailoring + cooking
	updateBar(self.barLearnerSystem, percentile(0, SBvars.LearnerSystemSkill, levels), getText("UI_ETW_CurrentValue") .. levels)
	updateBar(
		self.barSleepSystem,
	...
	if self.barInventoryTransferSystemWeight ~= nil or self.barInventoryTransferSystemItems ~= nil then
		...
		local yPosition = (self.labelAllThumbsWeightLose and self.labelAllThumbsWeightLose:getY())
			or (self.labelDisorganizedItemsLose and self.labelDisorganizedItemsLose:getY())
		self:drawRectBorder(
			lineStartPosition,
			yPosition - (FONT_HGT_SMALL / 4),
	...
	if self.barBravery ~= nil then
		...
		self:drawRectBorder(
			lineStartPosition,
			self.labelCowardlyLose:getY() - (FONT_HGT_SMALL / 4),
			self:getWidth() - lineStartPosition * 1.5,
			heightOfBox,
			self.DimmedTextColor.a,
			self.DimmedTextColor.r,
	...
	updateLabel(
		...
		getText("UI_trait_BodyWorkEnthusiast") .. ": " .. metalworking + mechanics .. "/" .. SBvars.BodyworkEnthusiastSkill
	)
	updateLabel(
		self.labelBodyWorkEnthusiastRepairsProgress,
		getText("UI_trait_BodyWorkEnthusiast") .. ": " .. modData.VehiclePartRepairs .. "/" .. SBvars.BodyworkEnthusiastRepairs
	)
	...
	updateLabel(
		...
		getText("UI_trait_Mechanics") .. ": " .. math.floor(modData.VehiclePartRepairs) .. "/" .. SBvars.MechanicsRepairs
	)
	...
	updateLabel(
		...
		getText("UI_trait_PetTherapy") .. ": " .. #modData.AnimalsSystem.UniqueAnimalsPetted .. "/" .. SBvars.PetTherapyUniqueAnimalsPetted
	)
	updateLabel(
