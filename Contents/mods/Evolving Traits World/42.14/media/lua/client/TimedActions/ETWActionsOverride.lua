...
function ISChopTreeAction:perform()
	if ETWCommonLogicChecks.AxemanShouldExecute() then
		...
		if modData.TreesChopped >= SBvars.AxemanTrees then
			...
			then
				player:getTraits():add(ETWTraitsRegistry.AXEMAN)
...
function ISInventoryTransferAction:perform()
	if ETWCommonLogicChecks.InventoryTransferSystemShouldExecute() and self.character == getPlayer() then
		...
		then
			...
			then
				player:getTraits():remove(CharacterTrait.DISORGANIZED)
				if notification() then
		...
		then
			...
			then
				player:getTraits():add(CharacterTrait.ORGANIZED)
				-- UI_trait_Packmule is internal string name
				if notification() then
		...
		then
			...
			then
				player:getTraits():remove(CharacterTrait.ALL_THUMBS)
				if notification() then
		...
		then
			...
			then
				player:getTraits():add(CharacterTrait.DEXTROUS)
				if notification() then
