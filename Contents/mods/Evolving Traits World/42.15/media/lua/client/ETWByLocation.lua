...
local function fearOfLocations(isKill)
	...
	if not SBvars.FearOfLocationsExclusiveFears then
		if
			not player:hasTrait(CharacterTrait.AGORAPHOBIC)
		...
		if
			not player:hasTrait(CharacterTrait.CLAUSTROPHOBIC)
	...
	if
		player:hasTrait(CharacterTrait.AGORAPHOBIC)
		and fearOfLocationsModData.FearOfOutside >= SBCounter
	...
	if
		player:hasTrait(CharacterTrait.CLAUSTROPHOBIC)
		and fearOfLocationsModData.FearOfInside >= SBCounter
		and SBvars.TraitsLockSystemCanLoseNegative
	then
		ETW_CommonFunctions.removeTraitFromPlayer(player, CharacterTrait.CLAUSTROPHOBIC)
