...
function ETW_WeatherTraits.sunSensitivityTrait(player, bodyDamage, modData)
	...
	local umbrellaMultiplier = math.max(0, math.min(1, SBvars.SunSensitivityUmbrellaMultiplier or 0.5))
	local daylightStrength = math.max(0, math.min(1, getClimateManager():getDayLightStrength()))
	...
	local resultingPain = math.max(0, math.min(100, baselinePain + desiredPain))
